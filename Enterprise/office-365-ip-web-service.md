...
끝점 웹 메서드는 Office 365 서비스를 구성하는 IP 주소 범위 및 URL에 대한 모든 레코드를 반환합니다. 끝점 웹 메서드의 최신 데이터는 네트워크 디바이스 구성에 사용되는 동안, 추가에 대해 제공된 사전 알림 때문에 게시되고 최대 30일 동안 캐시될 수 있습니다. 버전 웹 메서드가 새 버전의 데이터를 사용할 수 있다는 사실을 나타낼 경우에만 끝점 웹 메서드를 다시 호출하는 것이 좋습니다. 끝점 웹 메서드에 대한 매개 변수는 다음과 같습니다. 
...
끝점 웹 메서드를 호출하면 동일한 클라이언트 IP 주소에서 HTTP 응답 코드 429 ‘너무 많은 요청’이 응답할 수 없을만큼 많이 수신될 수 있습니다. 대부분의 사용자에게는 이 응답 코드가 표시되지 않습니다. 이 응답 코드가 표시되면 메서드를 다시 호출하기 전에 1시간 정도 기다려야 합니다. 버전 웹 메서드가 사용 가능한 새 버전이 있음을 나타낼 경우에만 끝점 웹 메서드를 호출하세요. 
...
변경 웹 메서드는 끝점 웹 메서드와 동일한 방식으로 속도가 제한됩니다. 429 HTTP 응답 코드가 표시되는 경우 1시간 정도 기다렸다가 다시 호출해야 합니다. 
...
- disposition - change, add 또는 remove일 수 있으며, 끝점 세트 레코드에 대해 수행된 변경을 설명합니다. 필수 항목입니다.
- impact - 모든 변경이 모든 환경에서 동일하게 중요한 것은 아닙니다. 이 요소는 이 변경의 결과로 엔터프라이즈 네트워크 주변 환경에 대해 예상되는 영향을 설명합니다. 이 특성은 버전 2018112800 이상의 변경 레코드에만 포함됩니다. impact에 대한 옵션은 다음과 같습니다.
  ...
  - AdedUrl - URL이 Office 365에 추가되었으며 곧 서비스될 예정입니다. 이 옵션은 프록시 서버 또는 URL 구문 분석 네트워크 주변 디바이스에서 수행해야 하는 변경을 나타냅니다. 사용을 시작하기 전에 이 옵션을 추가하지 않으면 중단이 발생할 수 있습니다.
