...
다중-지리적으로 분산 환경 설정 된, 하는 즉시 사용자 나타납니다 뉴스, 해당 SharePoint 홈 페이지에 여러 지리적 위치에서 최근 및 열어본 사이트입니다. SharePoint 홈 페이지에서 검색 상자를 사용 하는 경우에 여러 지리적 위치에서 병합 된 결과 얻을 수 있습니다.
...
1. 쿼리 속성 EnableMultiGeoSearch와 검색 REST 끝점을 호출 하는 검색 클라이언트 = true입니다.
2. 쿼리는 테 넌 트의 모든 지리적 위치에 전송 됩니다.
3. 각 지리적 위치에서 검색 결과 병합 하 고 순위가 지정 됩니다.
4. 클라이언트 통합된 검색 결과를 가져옵니다.



<span id="_Set_up_a" class="anchor"><span id="_Ref501388384" class="anchor"></span></span>모든 지리적 위치에서 결과 얻은 했을 때까지 검색 결과 병합 하지는 알 수 있습니다. 즉, 다중-지리적으로 분산 검색 하나만 지리적 위치를 사용 하는 환경에서 검색에 비해 대기 시간을 추가 합니다.

<span id="_Set_up_a_1" class="anchor"><span id="_Ref505252370" class="anchor"></span></span>
## <a name="get-a-search-center-to-show-results-from-all-geo-locations"></a>모든 지리적 위치에서 결과 표시 하도록 검색 센터 가져오기
...
3.  설정, 오른쪽, 위쪽 모서리에서 **설정** 기어 아이콘을 클릭 한 다음 **페이지 편집**을 클릭 하 여 세로 선택 합니다. 검색 결과 페이지가 편집 모드로 열립니다.

     ![](media/configure-search-for-multi-geo_image2.png)
1.  검색 결과 웹 파트에 포인터를 위, 웹 파트의 오른쪽 모서리는 화살표를 클릭 한 다음 메뉴에서 **웹 파트 편집** 을 클릭 합니다. 위쪽에서 리본 메뉴에서 검색 결과 웹 파트 도구 창이 열립니다 페이지의 오른쪽입니다.![](media/configure-search-for-multi-geo_image3.png)
...
MultiGeoSearchStatus – SharePoint 검색 API는 요청에 대 한 응답에서을 반환 하는 속성입니다. 속성의 값은 문자열 및 SharePoint 검색 API를 반환 하는 결과 대 한 다음 정보를 제공 합니다.
