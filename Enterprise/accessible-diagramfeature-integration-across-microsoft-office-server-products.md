...
## <a name="introduction-tab"></a>소개 탭

### <a name="illustrations-for-cross-server-features"></a>서버 간 기능에 대 한 그림

이 Visio 파일 (또는 multipage PDF 파일)에는 9 개의 탭이 있으며 각각에는 정보 및 Office Server 제품에서 작동 하는 기능에 대 한 다이어그램이 포함 되어 있습니다.
  
다음과 같은 기능 및 탭을 사용할 수 있습니다. 
...
이 솔루션에 대 한 의견을 보내거나 MODAContent@microsoft.com에 대 한 추가 솔루션을 요청 합니다. 
  
### <a name="tips-for-printing"></a>인쇄 팁

각 탭의 페이지 크기는 22 x 17 인치 (ANSI 공학 다이어그램 크기의 1/4)입니다. 이 페이지는 2 개의 tabloid 크기 용지 (17 x 11) 인치 또는 4 개의 letter 크기 용지 (11 x 8.5 인치)로 인쇄할 수 있습니다. 플로터가 있는 경우 이러한 포스터를 최대 크기로 인쇄할 수 있습니다. 플로터가 없으면 다음 단계에 따라 더 작은 용지에 인쇄하십시오. 
...
Windows phone을 사용 하거나 QR 코드 판독기를 다운로드 하 여 이러한 기능을 구현 하는 방법에 대 한 자세한 내용을 확인 하세요. 
  
### <a name="cross-server-features"></a>서버 간 기능

표에는 이러한 기능을 사용 하는 Office 서버 제품 및 기능이 나와 있습니다. 이러한 기능은 다음과 같습니다. 
  
서버 간 인증 이 기능은 다음에 적용할 수 있습니다. 
...
통합 연락처 저장소 이 기능은 다음에 적용할 수 있습니다. 
  
- Exchange
    
- Lync
    
사이트 사서함 이 기능은 다음에 적용할 수 있습니다. 
  
- SharePoint
    
- Exchange
    
Exchange 작업 동기화 이 기능은 다음에 적용할 수 있습니다. 
  
- SharePoint
    
- Exchange
    
Outlook Web App의 Lync 현재 상태 이 기능은 다음에 적용할 수 있습니다. 
  
- Lync
    
음성 메일. 이 기능은 다음에 적용할 수 있습니다. 
  
- Lync
    
모임 녹음/녹화 이 기능은 다음에 적용할 수 있습니다. 
...
서버 간 인증 트러스트를 구성 하려면 사용자를 대신 하 여 리소스 요청을 전송 하는 각 서버에 해당 하는 신뢰할 수 있는 새 보안 토큰 발급자를 추가 해야 합니다. 각 서버 유형에는 구성 정보 및 액세스 토큰 서명 인증서의 공개 부분을 포함 하는 JSON (JavaScript Object Notation) 메타 데이터 끝점이 있습니다. 서버 간 인증 트러스트를 구성 하는 과정에서 다른 서버의 JSON 메타 데이터 끝점을 지정 하는 것이 좋습니다. 
...
- Exchange 2013와 SharePoint 2013 사이 
    
- Exchange 2013와 Lync 2013 간의 두 가지입니다. 
...
사용자는 픽셀 범위가 48 × 48에서 648 × 648 인 그림을 업로드할 수 있습니다. 사진 크기 조정: 
...
함께 제공 되는 다이어그램은 사용자가 모든 Office 응용 프로그램에서 사용할 수 있는 사진을 업데이트 하는 방법을 보여 줍니다. 
  
1. 사용자가 Outlook, SharePoint 또는 Lync의 사진을 업데이트 합니다. 업데이트 된 사진은 모든 Office 응용 프로그램에서 사용 됩니다. 
...
## <a name="unified-contact-store-tab"></a>통합 연락처 저장소 탭
...
### <a name="a-central-filing-cabinet-for-emails-and-documents"></a>전자 메일 및 문서에 대 한 중앙 관리 캐비닛

사이트 사서함은 동일한 클라이언트 인터페이스를 사용 하 여 Exchange에 저장 된 전자 메일 메시지와 SharePoint에 저장 된 문서에 액세스할 수 있도록 하 여 공동 작업 및 사용자 생산성을 향상 시킵니다. 
...
- SharePoint 팜에서 사용자 프로필 동기화를 구성 합니다. 
    
- SharePoint 팜에서 앱 관리 서비스 응용 프로그램을 구성 합니다. 
    
- 서버 간 인증을 지원 하도록 기본 영역에 대해 SSL을 구성 합니다. 
    
- SharePoint 2013를 실행 하는 서버에 EWS API를 설치 합니다. 
    
- SharePoint 2013를 실행 하는 서버에 대해 OAuth 트러스트 및 서비스 권한을 설정 합니다. 
    
Exchange 구성: 
  
- Exchange 서버에 대 한 OAuth 트러스트 및 서비스 권한을 설정 합니다. 
    
- 사이트 사서함 프로 비전 정책을 만듭니다. 
    
- 사이트 사서함 이름 접두사를 구성 합니다 (선택 사항). 
    
 **작동 방법**
  
사이트 사서함은 기능적으로 SharePoint 2013 사이트 구성원(소유자 및 구성원), 전자 메일 메시지의 경우 Exchange 2013 사서함을 통한 공유 저장소, 문서의 경우 SharePoint 2013 사이트를 통한 공유 저장소 및 프로비전과 수명 주기 요구를 처리하는 관리 인터페이스로 구성되어 있습니다. 
  
함께 제공 되는 다이어그램은 사이트 사서함을 사용 하 여 Outlook 및 SharePoint에 저장 된 문서에 액세스 하는 사용자를 보여줍니다. 
  
1. 사용자는 Outlook 2013 Pro Plus의 사이트 사서함을 통해 SharePoint 팀 사이트 문서에 액세스할 수 있습니다. 
    
2. 사용자는 SharePoint 팀 사이트에서 사이트 사서함 받은 편지함의 전자 메일을 읽을 수도 있습니다. 
    
3. 전자 메일은 Exchange 서버의 사이트 사서함에 저장 됩니다. 
    
4. 문서는 SharePoint 서버의 사이트 사서함에 저장 됩니다. 
    
5. SharePoint 사이트의 콘텐츠 메타 데이터는 HTTPS를 통한 REST (표현 상태 전송) API를 사용 하 여 Exchange와 동기화 됩니다. 
    
### <a name="provisioning-and-management"></a>프로비저닝 및 관리

사이트 사서함은 SharePoint 2013을 통해 프로 비전 되 고 관리 됩니다. 구축 및 관리를 위한 SharePoint 및 Exchange 기능이 있습니다. 
  
#### <a name="sharepoint"></a>SharePoint

다이어그램에는 다음을 비롯 하 여 사이트 사서함을 구축 하는 데 필요한 SharePoint의 구성 요소가 나와 있습니다. 
...
새 사이트 사서함을 구축 하려면 팀 사이트에 사이트 사서함 앱을 설치 하 고 앱에 최소한 한 번 액세스 합니다. 
  
SharePoint 사이트 구성원 자격은 사이트 사서함에 액세스할 수 있는 사람을 결정 합니다. 
  
사이트 사서함 보존은 연결 된 SharePoint 사이트에 대해 구성 된 것과 동일한 수명 주기 정책을 따릅니다. 
  
#### <a name="exchange"></a>Exchange

이 다이어그램에는 사이트 사서함 프로 비전 정책이 나와 있습니다. 이는 사이트 사서함을 구축 하는 데 필요한 Exchange 구성 요소입니다.
  
Exchange 서버에서 사이트 사서함 프로 비전 정책을 정의할 수 있습니다. 이러한 정책은 사이트 사서함에 대해 송수신 되는 전자 메일 특성, Exchange 서버의 사이트 사서함 크기 및 사이트 사서함 전자 메일 주소에 대 한 접두사를 정의할 수 있도록 제어 합니다. 
  
온-프레미스 Exchange 배포의 경우 SharePoint 수명 주기 정책을 통해 삭제 하도록 표시 된 사이트 사서함을 주기적으로 검색 하 고 삭제 해야 합니다. 
...
Exchange 2013에서 다음을 수행 합니다. 
  
- OAuth 트러스트 및 서비스 권한을 구성 합니다. 
    
SharePoint Server 2013에서 다음을 수행 합니다. 
  
- User Profile Service 응용 프로그램 
    
- 작업 관리 서비스 응용 프로그램 
    
- 검색 (SharePoint Server 2013의 작업에 필요) 연속 크롤링 및 증분 크롤링을 사용 하 여 구성 합니다. 
    
- SSL (Secure Sockets layer)이 필요 합니다. 
...
Project Server 2013에서 다음을 수행 합니다. 
...
- 작업 관리 서비스 응용 프로그램 (아래 설명 참조) 
    
- Exchange 동기화 타이머 작업에 대해 아래에서 설명 합니다. 
    
- SharePoint Server 2013에는 사용자의 내 사이트 및 기타 사이트가 포함 되며 다양 한 사용자 작업을 실행 합니다. 
    
- SharePoint Server 2013에는 검색 인덱스가 포함 되어 있습니다. 
    
Exchange 서버 2013에는 다음이 포함 됩니다. 
  
- 사용자 전자 메일 정보가 있는 Exchange 데이터베이스 
    
- 동기화 작업 
    
Outlook 2013에는 다음이 표시 됩니다. 
  
- 사용자는 아래에서 설명 하는 작업을 동기화 하도록 선택할 수 있습니다. 
    
- 사용자는 Outlook에서 작업을 보고 편집할 수 있습니다. 
    
Project Server 2013에는 다음이 표시 됩니다. 
  
- Project 데이터베이스 
    
- 작업을 포함 하는 Project Web Access 사이트 
    
작업 관리 서비스 응용 프로그램은 다음과 같습니다. 
  
- SharePoint 목록 및 프로젝트 작업 목록에서 작업을 집계 하 여 작업을 Exchange Server와 동기화 하지 않습니다. 
    
- 사용자가 내 사이트를 볼 때 동기화 합니다. 
    
- 옵트인 하는 사용자의 목록을 유지 관리 합니다. 
    
- 다음 사용자 일괄 처리를 동기화 합니다. 
    
Exchange 동기화 타이머 작업은 다음과 같습니다. 
  
- 다음 사용자 일괄 처리를 결정 합니다. 
    
- 모든 사용자가 지속적으로 동기화 되도록 합니다. 
    
- 작업 관리 서비스 응용 프로그램에 대 한 호출을 시작 하 여 옵트인 전용 사용자에 대해 작업을 Exchange Server와 동기화 합니다. 
...
사용자의 로컬 컴퓨터에 Lync 2013이 설치 되어 있으면 Outlook 및 SharePoint Server에서 사용자의 현재 상태 정보를 자동으로 표시 합니다. 
  
Outlook Web App 사용자의 경우 Exchange CAS는 사용자를 대신 하 여 현재 상태를 쿼리 합니다. 
...
5. B. Internet Explorer는 현재 상태 정보를 검색 하기 위해 브라우저와 동일한 컴퓨터에 설치 되어 있는 Lync를 호출 합니다. 
    
## <a name="voicemail-tab"></a>음성 메일 탭

### <a name="exchange-um-is-the-voicemail-system-for-lync-server"></a>Exchange UM은 Lync Server에 대 한 음성 메일 시스템입니다.

음성 메일은 발신자가 Exchange UM (통합 메시징)을 사용 하 여 Lync 사용자에 게 음성 메일을 남길 수 있도록 허용 합니다. 
  
클라이언트 제품: 
  
- Lync 2013 
    
- PSTN 장치 (PBX, 셀룰러, POTS) 
...
- 발신자가 PSTN 장치 (PBX, 셀룰러, POTS)를 사용 하 여 전화를 시작 합니다. 
    
발신자가 Lync 2013을 사용 하 여 전화를 시작 합니다. 
  
1. 발신자는 Lync 2013를 사용 하 여 수신자에 대 한 호출을 시작 합니다. 통화가 시작 되어 Lync Server로 전송 됩니다. 
...
1. 발신자 B가 PSTN을 사용 하 여 수신자의 전화 번호로 전화를 겁니다. 
    
2. PSTN 통화는 IP 게이트웨이에서 중재 서버 (Lync server)로 라우팅됩니다. 
...
### <a name="publish-your-meeting-recordings-on-your-sharepoint-team-site"></a>SharePoint 팀 사이트에 모임 기록 게시

모임 녹음/녹화는 통합 커뮤니케이션의 핵심 구성 요소입니다. 모임 녹음을 공유 하는 좋은 방법은 팀 사이트에서 SharePoint 자산 라이브러리를 사용 하 여 모임 기록을 저장 하는 것입니다. 
...
다음은 모임 중에 MP4 파일에 기록 되며 각 글머리 기호 앞에는 녹음 형식을 나타내는 아이콘이 표시 됩니다. 
...
* 모임 내의 인스턴트 메시지만 포함 됩니다. 모임 참가자 간의 모든 피어 투 피어 메시징은 모임에 포함 되지 않으므로 캡처되지 않습니다. 
  
포스터에는 두 가지 서로 다른 시나리오에 대 한 두 가지 다이어그램이 포함 되어 있습니다. 
  
- 모임 기록 게시 준비 
    
- Lync 클라이언트를 사용 하 여 모임 기록 및 게시 
...
- 팀 구성원이 모임 녹음/녹화를 보내는 모임 자산 라이브러리 
...
1. SharePoint 팀 사이트에서 자산 라이브러리 앱을 추가 합니다. 필요한 경우 크기 제한 또는 연결 시간 제한으로 인해 모임 기록을 업로드할 수 없는 경우 2 단계와 3 단계의 추가 작업을 수행 합니다. 
...
이 다이어그램은 Lync를 사용 하 여 모임에 참가 하는 사용자를 보여 줍니다. 모임이 Lync 클라이언트를 사용 하 여 기록 되며,이는 모임 콘텐츠를 사용 하 여 MP4 파일을 만듭니다. MP4 녹음은 컴퓨터의 Lync 기록 폴더에 저장 됩니다. MP4 기록을 wiki, SharePoint 페이지 또는 블로그에 삽입할 수 있는 모임 자산 라이브러리로 이동할 수 있습니다. 
  
 **Lync 클라이언트를 사용 하 여 모임 기록 및 게시**
  
1. Lync 클라이언트를 사용 하 여 모임 녹음을 시작 합니다. 
