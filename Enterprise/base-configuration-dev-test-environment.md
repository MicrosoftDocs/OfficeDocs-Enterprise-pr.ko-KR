...
이 문서에서는 Azure에서 다음과 같은 기본 구성 개발/테스트 환경을 만들기 위한 단계별 지침을 제공합니다.
...
- 추가 가상 머신, Azure 서비스 또는 기타 Microsoft 클라우드 서비스(예: Office 365 및 EMS(Enterprise Security + Mobility))가 포함된 직접 디자인한 확장된 테스트 환경의 초기 구성으로
    
Azure에서 기본 구성 테스트 환경을 설정하는 과정은 다음 4단계로 진행됩니다.
...
## <a name="phase-1-create-the-virtual-network"></a>1단계: 가상 네트워크 만들기
...
### <a name="connect-to-dc1-using-local-administrator-account-credentials"></a>로컬 관리자 계정 자격 증명을 사용하여 DC1에 연결
...
DC1이 다시 시작되면 DC1 가상 컴퓨터에 다시 연결합니다.
  
### <a name="connect-to-dc1-using-domain-credentials"></a>도메인 자격 증명을 사용하여 DC1에 연결
...
CLIENT1은 Contoso 인트라넷에서 일반적인 랩톱, 태블릿 또는 데스크톱 컴퓨터의 역할을 합니다.
...
### <a name="verify-client-access-to-app1"></a>APP1에 대한 CLIENT 액세스 확인
